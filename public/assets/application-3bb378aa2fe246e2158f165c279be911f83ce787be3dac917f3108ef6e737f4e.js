...
(function() {
  $(document).on('turbolinks:load', function() {
    return $('.js-favorite-button').on('ajax:success', function(event) {
      var status;
      ...
      switch (status) {
        case "created":
          ...
          $(this).attr({
            'data-method': 'delete',
            'data-confirm': "お気に入り登録を解除してもよろしいですか？",
            href: event.detail[0]['delete_path']
          });
          return $(this).children().removeClass('grey-text').addClass('yellow-text');
        case "unprocessable_entity":
          return Materialize.toast('お気に入り登録に失敗しました', 4000);
        case "already_added_to_favorite":
        ...
        case "deleted":
          ...
          $(this).attr({
            'data-method': 'post',
            href: event.detail[0]['post_path']
          }).removeAttr('data-confirm');
          return $(this).children().removeClass('yellow-text').addClass('grey-text');
      ...
      return Materialize.toast('サーバーとの通信に失敗しました。しばらく時間を置いてから再度お試しください。', 4000);
    });
  });
...
(function() {
  $(document).on('turbolinks:load', function() {
    return $('#js-inquiry-form').on('ajax:success', function(event) {
      var errors, status;
      status = event.detail[0]['status'];
      switch (status) {
        case "created":
          ...
          return $('#js-inquiry-form-success-message-container').removeClass('hide');
        case "unprocessable_entity":
          Materialize.toast('問い合わせに失敗しました', 4000);
          errors = event.detail[0]['data'];
          if (errors["inquirers_email"] != null) {
          ...
          if (errors["message"] != null) {
            return $("#error-message-for-inquiry-message-field").removeClass('hide');
          }
      }
    }).on('ajax:error', function(event) {
      $('#js-inquiry-form-container').addClass('hide');
      return $('#js-inquiry-form-error-message-container').removeClass('hide');
    });
  });

}).call(this);
$(document).on('turbolinks:load', function() {
  $('.datepicker#js-birthday-picker').pickadate({
  ...
  $('.datepicker#js-event-data-picker').pickadate({
    selectMonths: true, // Creates a dropdown to control month
    ...
    close: "閉じる",
    format: "yyyy-mm-dd",
    closeOnSelect: true, // Close upon selecting a dat
  });

  $('.datepicker#js-application-period').pickadate({
    selectMonths: true, // Creates a dropdown to control month
    selectYears: 6, // Creates a dropdown of 15 years to control year
    min: new Date(),
    monthsFull:  ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"],
...
(function() {
  var decrementParticipantCount, incrementParticipantCount;

  $(document).on('turbolinks:load', function() {
    return $('.js-participate-button').on('ajax:success', function(event) {
      var status;
      status = event.detail[0]['status'];
      switch (status) {
        case "created":
          ...
          return $(this).html('<i class="material-icons grey-text">group</i><br>辞退');
        case "unprocessable_entity":
          return Materialize.toast('参加申請に失敗しました', 4000);
        ...
        case "resign_completed":
          ...
          return $(this).html('<i class="material-icons red-text text-accent-2">group_add</i><br>参加');
        case "already_resigned":
          return Materialize.toast('既に辞退しています', 4000);
      }
    }).on('ajax:error', function(event) {
      return Materialize.toast('サーバーとの通信に失敗しました。しばらく時間を置いてから再度お試しください。', 4000);
    });
  ...
  incrementParticipantCount = function($capacity_count_disp) {
    var $capacity_num, $capacity_num_disp, $participant_num, $participant_num_disp;
    ...
    if ($participant_num >= $capacity_num) {
      return $participant_num_disp.attr('class', 'js-participant-num-disp u-fz14 red-text text-accent-2');
    }
  };

  decrementParticipantCount = function($capacity_count_disp) {
    var $capacity_num, $capacity_num_disp, $participant_num, $participant_num_disp;
    $participant_num_disp = $capacity_count_disp.find('.js-participant-num-disp');
    $participant_num = ($participant_num_disp.text() * 1) - 1;
    $capacity_num_disp = $capacity_count_disp.children().find('.js-capacity-num-disp');
    ...
    $participant_num_disp.text($participant_num);
    if ($participant_num < $capacity_num) {
      return $participant_num_disp.attr('class', 'js-participant-num-disp u-fz11 light-blue-text');
    }
  };
